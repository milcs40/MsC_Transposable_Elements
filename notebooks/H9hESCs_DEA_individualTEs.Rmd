...
countData <- fread(file = "analysis/featureCounts/UniqueMode/UniqueMode_genes_gencode_all_rmsk_perIndivRepeat.counts", header = T, sep = "\t", data.table = FALSE)
...
countData_simple_TEs
```

```{r}
plot(density(log10(apply(countData_simple_TEs, 1, sum))))
```
```{r}
#countData_simple_TEs_filtered <- countData_simple_TEs[rowSums(countData_simple_TEs) > 500, ]
countData_simple_TEs_filtered <- countData_simple_TEs[rowSums(countData_simple_TEs) > 2000, ]
#countData_simple_TEs_filtered <- countData_simple_TEs[rowSums(countData_simple_TEs) > 5000, ]
#countData_simple_TEs_filtered <- countData_simple_TEs[rowSums(countData_simple_TEs > 100) >= 6 & rowSums(countData_simple_TEs) > 1000, ]
#countData_simple_TEs_filtered <- countData_simple_TEs[rowSums(countData_simple_TEs > 500) >= 6 & rowSums(countData_simple_TEs) > 5000, ]
```
```{r}
plot(density(log10(apply(countData_simple_TEs_filtered, 1, sum))))
```
...
#countData_simple <- countData_simple[rowSums(countData_simple) > 100, ]
#countData_simple <- countData_simple[rowSums(countData_simple) > 2000, ]
#countData_simple <- countData_simple[rowSums(countData_simple > 500) >= 6 & rowSums(countData_simple) > 5000, ]

head(countData_simple, 10)
#View(countData_simple)
```
```{r}
plot(density(log10(apply(countData_simple, 1, sum))))
```
...
plot <- pheatmap(distance_rlogdds_matrix,
...
```{r, fig.height=6, fig.width=7}
pca_uniquely_mapped_plot <- biplot(prlog,
       # loadings parameters
       showLoadings = TRUE,
...
Let's now look at PC2 and PC3, to see if the 3rd component can explain some biological variability.
```{r, fig.height=6, fig.width=7}
pca_uniquely_plot_pc2_pc3 <- biplot(prlog,
       x = 'PC2', 
       y = 'PC3',
       # loadings parameters
       showLoadings = F,
...
pca_uniquely_plot_pc2_pc3
```
Make a panel with PC2/PC2 for multimapped and uniquely mapped strategies (requires running both markdowns)
```{r, fig.height=6, fig.width=12}
# Adjust the vjust to lower the labels
final_panel <- ggarrange(
  pca_multimapped_plot_pc2_pc3 + theme(legend.position = "right"),  # First plot without changing the legend
  pca_uniquely_plot_pc2_pc3 + theme(legend.position = "none"),  # Second plot without legend
  labels = c("A", "B"),    # Labels for the plots
  font.label = list(size = 16, face = "bold"),  # Lower the labels using vjust
  ncol = 2,                # Number of columns (plots side by side)
...
suppressMessages(pairsplot(prlog,
          components = getComponents(prlog, c(1:5)),
          triangle = TRUE, 
          trianglelabSize = 12,
          hline = 0, 
          vline = 0,
...
# Now, we only have to plot the volcano plot.
volcano_individual <- ggplot(results_ordered_DESeq2_naiveVsPrimed_Wald) +
  theme_bw() +
...
results_ordered_DESeq2_naiveVsPrimed_TEs_Wald$genelabels[1:8] <- rownames(results_ordered_DESeq2_naiveVsPrimed_TEs_Wald)[1:8]
...
# Now, we only have to plot the volcano plot.
volcano_TEs_individual <- ggplot(results_ordered_DESeq2_naiveVsPrimed_TEs_Wald) +
  theme_bw() +
...
```{r, fig.height=8, fig.width=7}
pheatmap::pheatmap(normalized_significant_genes_naiveVsPrimed,
         cluster_rows = T,
         cluster_cols = T,
         show_rownames = T,
         border_color = NA,
...
heatmap_genes_individual
```
```{r, fig.height=7, fig.width=12}
# Arrange the plots side by side with labels
final_panel <- ggarrange(
  volcano_individual, heatmap_genes_individual$gtable,  # Your two plots
  labels = c("A", "B"),                      # Labels for the plots
...
# normalized_significant_TEs_naiveVsPrimed <- countsDESeq2NormLog2[rownames(significant_genes_naiveVsPrimed_TE_pheatmap), ]
# 
...
heatmap_TEs_individual <- pheatmap(normalized_significant_TEs_naiveVsPrimed,
         cluster_rows = T,
         cluster_cols = T,
         show_rownames = T,
         border_color = NA,
...
heatmap_TEs_individual
```
```{r, fig.height=7, fig.width=12}
# Arrange the plots side by side with labels
final_panel_TEs_individual <- ggarrange(
  volcano_TEs_individual, heatmap_TEs_individual$gtable,  # Your two plots
  labels = c("A", "B"),                      # Labels for the plots
  font.label = list(size = 16, face = "bold", hjust = -1, vjust = 1.5),  # Adjust label position
  ncol = 2,                                  # Number of columns
  widths = c(.9, 1)                              
...
```{r}
results_down_naiveVsPrimed_Genes <- res_genotype_state_naiveVsPrimed_interaction[!grepl("TE_",rownames(res_genotype_state_naiveVsPrimed_interaction)), ]

results_down_naiveVsPrimed_Genes <- results_down_naiveVsPrimed_Genes[which(results_down_naiveVsPrimed_Genes$log2FoldChange < -1 & results_down_naiveVsPrimed_Genes$padj < 0.01), ] # Get the downregulated genes
results_down_naiveVsPrimed_Genes_sorted <- results_down_naiveVsPrimed_Genes[order(results_down_naiveVsPrimed_Genes$log2FoldChange), ] # Sort your results by padj
top30_results_down_naiveVsPrimed_Genes_sorted <- rownames(results_down_naiveVsPrimed_Genes_sorted[1:30, ]) # Select the top 30 genes
#top30_results_down_naiveVsPrimed_sorted <- rownames(results_down_naiveVsPrimed_sorted) # Select all genes

top30_results_down_naiveVsPrimed_Genes_sorted_df <- data.frame(countsDESeq2NormLog2[top30_results_down_naiveVsPrimed_Genes_sorted, ]) # Extract the normalized read counts for the top 20 genes, from the normalized counts
top30_results_down_naiveVsPrimed_Genes_sorted_df$gene <- rownames(top30_results_down_naiveVsPrimed_Genes_sorted_df) # Create a columns with the gene names, from the rownames of the table
top30_results_down_naiveVsPrimed_Genes_sorted_df_melt <- reshape2::melt(top30_results_down_naiveVsPrimed_Genes_sorted_df) # Melt the dataframe, into a format that can be read by ggplot2.
colnames(top30_results_down_naiveVsPrimed_Genes_sorted_df_melt) <- c("gene", "samplename", "normalized_counts") # Add column names that make sense

top30_meta <- sample_info # create a meta info
top30_meta$samplename <- rownames(top30_meta) # create a column with sample name

top30_results_down_naiveVsPrimed_Genes_sorted_df_melt <- merge(top30_results_down_naiveVsPrimed_Genes_sorted_df_melt, top30_meta) # melt the two tables. Merged on samplename

ggplot(top30_results_down_naiveVsPrimed_Genes_sorted_df_melt) +
  geom_point(aes(x = gene, y = normalized_counts, color = state), position=position_jitter(w=0.1,h=0), size = 1.5) +
  #scale_y_log10() +
  xlab("Genes") +
  ylab("Log2 Normalized Counts") +
  ggtitle("Top Significantly downregulated Genes in Naive Vs. Primed\n(sorted by log2FoldChange), padj < 0.01, log2FoldChange < -1") +
  theme_bw() +
...
```{r}
results_up_naiveVsPrimed_Genes <- res_genotype_state_naiveVsPrimed_interaction[!grepl("TE_",rownames(res_genotype_state_naiveVsPrimed_interaction)), ]

results_up_naiveVsPrimed_Genes <- results_up_naiveVsPrimed_Genes[which(results_up_naiveVsPrimed_Genes$log2FoldChange > 1 & results_up_naiveVsPrimed_Genes$padj < 0.01), ] # Get the downregulated genes
results_up_naiveVsPrimed_Genes_sorted <- results_up_naiveVsPrimed_Genes[order(results_up_naiveVsPrimed_Genes$log2FoldChange, decreasing = T), ] # Sort your results by padj
top30_results_up_naiveVsPrimed_Genes_sorted <- rownames(results_up_naiveVsPrimed_Genes_sorted[1:30, ]) # Select the top 20 genes
#top30_results_up_naiveVsPrimed_Genes_sorted = rownames(results_up_naiveVsPrimed_Genes_sorted) # Select all top genes

top30_results_up_naiveVsPrimed_Genes_sorted_df <- data.frame(countsDESeq2NormLog2[top30_results_up_naiveVsPrimed_Genes_sorted, ]) # Extract the normalized read counts for the top 20 genes, from the normalized counts
top30_results_up_naiveVsPrimed_Genes_sorted_df$gene <- rownames(top30_results_up_naiveVsPrimed_Genes_sorted_df) # Create a columns with the gene names, from the rownames of the table
top30_results_up_naiveVsPrimed_Genes_sorted_df_melt <- reshape2::melt(top30_results_up_naiveVsPrimed_Genes_sorted_df) # Melt the dataframe, into a format that can be read by ggplot2.
colnames(top30_results_up_naiveVsPrimed_Genes_sorted_df_melt) <- c("gene", "samplename", "normalized_counts") # Add column names that make sense

top30_meta <- sample_info # create a meta info
top30_meta$samplename <- rownames(top30_meta) # create a column with sample name

top30_results_up_naiveVsPrimed_Genes_sorted_df_melt <- merge(top30_results_up_naiveVsPrimed_Genes_sorted_df_melt, top30_meta) # melt the two tables. Merged on samplename

ggplot(top30_results_up_naiveVsPrimed_Genes_sorted_df_melt) +
  geom_point(aes(x = gene, y = normalized_counts, color = state), position=position_jitter(w=0.1,h=0), size = 1.5) +
  #scale_y_log10() +
  xlab("Genes") +
  ylab("Log2 Normalized Counts") +
  ggtitle("Top Significantly upregulated Genes in Naive Vs. Primed\n(sorted by log2FoldChange), padj < 0.01, log2FoldChange > 1") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.title=element_text(hjust=0.5))
```
...
ggplot(top30_results_down_naiveVsPrimed_TEs_sorted_df_melt) +
  ...
  xlab("TEs") +
  ylab("Log2 Normalized Counts") +
  ggtitle("Top Significantly downregulated TEs in Naive Vs. Primed\n(sorted by Wald Statistics), padj < 0.01, log2FoldChange < -1") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.title=element_text(hjust=0.5))
```
...
ggplot(top30_results_up_naiveVsPrimed_TEs_sorted_df_melt) +
  ...
  xlab("TEs") +
  ylab("Log2 Normalized Counts") +
  ggtitle("Top Significantly upregulated TEs in Naive Vs. Primed\n(sorted by Wald Statistics), padj < 0.01, log2FoldChange > 1") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.title=element_text(hjust=0.5))
```
